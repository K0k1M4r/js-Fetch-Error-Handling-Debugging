...
function getInitials(firstName, lastName) {
  ...
  // 5 es saxshi gasarkvevi
  
  async function searchPokemon(name) {
    const pokemonName = String(name).toLowerCase().trim();
    
    // FIXED: Added the proper API path and confirmed backtick template usage
    const url = `https://pokeapi.co{pokemonName}`;
  
    try {
      const response = await fetch(url);
  
      // If the server returns a bad status code (like 404), throw an error
      if (!response.ok) {
